...
public class DateTimePicker extends JDialog {
    ...
    private JComboBox<Integer> secondsBox;
    private Consumer<LocalDateTime> callback;

    public DateTimePicker(Consumer<LocalDateTime> callback, LocalDateTime initialValue) {
        ...
        hourBox.setModel(new DefaultComboBoxModel<>(IntStream.range(0, 24).boxed().toArray(Integer[]::new)));
        minuteBox.setModel(new DefaultComboBoxModel<>(IntStream.range(0, 60).boxed().toArray(Integer[]::new)));
        secondsBox.setModel(new DefaultComboBoxModel<>(IntStream.range(0, 60).boxed().toArray(Integer[]::new)));
    ...
    private void onOK() {
        callback.accept(LocalDateTime.of(
                1900 + yearBox.getSelectedIndex(),
                monthBox.getSelectedIndex() + 1,
                dayBox.getSelectedIndex() + 1,
                hourBox.getSelectedIndex(),
                minuteBox.getSelectedIndex(),
                secondsBox.getSelectedIndex()));
